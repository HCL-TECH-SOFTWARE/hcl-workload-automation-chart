...
# HCL Workload Automation Z-Centric Agent on Kubernetes 
...
The information in this README contains the steps for deploying the following HCL workload Automation components using an Helm chart and contaner images: 
- z-Centric Agent 

For more information about HCL Workload Automation, see the product documentation library in [HCL Documentation](https://help.hcltechsw.com/workloadautomation/v101/welcome_zos.html).
...
You can access the HCL Workload Automation chart and container images from the Entitled Registry. See [Create the Entitlement Secret](#create-the-secret) for more information about accessing the registry. The images are as follows:

* hclcr.io/wa/hcl-workload-automation-agent-dynamic:10.1.0.01.20221130
...
- An API Entitlement key for accessing HCL Entitled Registry: `cp.icr.io`
...
## Installing
Installing and configuring the HCL Workload Automation components, involves the following high-level steps:

1. [Creating the Namespace](#create-the-namespace).
2. [Creating an Entitlement Secret](#create-the-secret)  to store on your cluster an entitlement key for the HCL Workload Automation offering.
...
To deploy the HCL Workload Automation components, ensure you have first downloaded the chart from the HCL Entitled Registry: `cp.icr.io` and have unpacked it to a local directory. If you already have the chart then update it.
...
1. Download the chart from the repository and unpack it to a local directory or, if you already have the chart, update it.
   ...
   **First time installation and configuration of the chart:**
   ...
   a. Add the repository:
        ...
        helm repo add <repo_name> https://workloadautomation.github.io/HCL-workload-automation-chart/stable, where <repo_name> represents the name of the chosen local repository
...
| waagent.image.repository                         | HCL Workload Automation Agent image repository                                                                                                                                                                                                                                | yes            | hclcr.io/wa/hcl-workload-automation-agent        | hclcr.io/wa/hcl-workload-automation-agent        |
| waagent.image.tag                                | HCL Workload Automation Agent image tag                                                                                                                                                                                                                                       | yes            | 10.1.0.01.20221130                        | 10.1.0.01.20221130                        |
| waagent.image.pullPolicy                         | image pull policy                                                                                                                                                                                                                                                    | yes            | Always                           | Always                           |
| waagent.licenseType                            | Product license type (HCL Workload Scheduler only)                                                                                                                                                                                                                                                             | yes           | PVU                           | PVU                       |
...
The HCL Workload Automation agents can use two different ways to route external traffic into the Kubernetes Service cluster:
...
By default a single agent is installed. **Only for dynamic agents:** If you want to change the topology for HCL Workload Automation, increase or decrease the values of the `replicaCount` parameter in the `values.yaml` file for each agent and save the changes.

#### Scaling up or down (only for dynamic agents)

To scale one or more HCL Workload Automation agents up or down:
...
If you use customized certificates, `useCustomizedCert:true`, you must create a secret containing the customized files that will replace the agent default ones in the \<workload_automation_namespace>. Customized files must have the same name as the default ones.
 ...
 ```   
   ...
   For details about custom certificates, see [Connection security overview](https://help.hcltechsw.com/workloadautomation/v101/search.html?searchQuery=customizing+certificates).
...
HCL Workload Automation requires persistent storage for each agent that you deploy, to maintain the scheduling workload and topology.
...
To access the complete product documentation library for HCL Workload Automation, see https://help.hcltechsw.com/workloadautomation/v101/index.html .


## Troubleshooting


In the event a problem should occur while using HCL Workload Automation, Customer Support might ask you to supply information about your system and environment to perform problem determination. The following utilities are available:

-   A general data capture utility command that extracts information about HCL Workload Automation  and related agent workstations, system-specific information, and data related to WebSphere Application Server Liberty Base; see [Data capture utility](https://help.hcltechsw.com/workloadautomation/v101/index.html).

-   A first failure data capture (ffdc) facility built into **batchman** and **mailman** that automatically runs the data capture utility when failures occur in **jobman**, **mailman**, or **batchman** and collects products logs, traces and configuration files; see [First failure data capture (ffdc)](https://help.hcltechsw.com/workloadautomation/v101/index.html).

-   The data capture utility script, wa_pull_info, is also used to collect data related to the Dynamic Workload Console to assist in problem determination; see [Data capture utility]( https://help.hcltechsw.com/workloadautomation/v101/index.html).

-   WebSphere Application Server Liberty Base javadump command to create the heap dump for WebSphere Application Server Liberty Base that runs on the Dynamic Workload Console and the master domain manager; see [Creating application server dumps]( https://help.hcltechsw.com/workloadautomation/v101/index.html).


In case of problems related to deploying the product with containers, see [Troubleshooting]( https://help.hcltechsw.com/workloadautomation/v101/index.html).
